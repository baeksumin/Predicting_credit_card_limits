...
summary(rating)
#rating 제거했을 때 회귀분석 0.9755

balance<-lm(Limit~Income+Rating+Cards+Student,data=Credit)
summary(balance)
#balance 제거했을 때 회귀분석 0.9956

#수정 r제곱 값이 balance을 제거한 모델에서 더 크기 때문에 balance을 제거하기로 한다.
...
best.fit<-lm(Limit~Income+Rating+Cards+Student,data=Credit)
summary(best.fit)
